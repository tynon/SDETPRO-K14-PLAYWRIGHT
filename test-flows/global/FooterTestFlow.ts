...
export default class FooterTestFlow {
    ...
    private async verifyInformationColumn(footerComponent: FooterComponent): Promise<void> {
        ...
        const expectdHrefs: string[] = ['/sitemap','/shipping-returns','/privacy-policy','/conditions-of-use','/about-us','/contactus'];
        const informationColumnComp: InformationColumnComponent = footerComponent.informationColumnComponent();
        await this.verifyFooterColumn(informationColumnComp, expectedLinkTexts, expectdHrefs);
    }
    ...
    private async verifyFooterColumn(
        ...
        deepStrictEqual(actualLinkTexts, expectedLinkTexts,
            `Actual link texts and expected link texts are not the same.
            Actual : ${actualLinkTexts}
            Expected : ${expectedLinkTexts}`
        )
        deepStrictEqual(actualHrefs, expectdHrefs,
            `Actual hrefs and expected hrefs are not the same.
            Actual : ${actualHrefs}
            Expected : ${expectdHrefs}`
        )
    }
}